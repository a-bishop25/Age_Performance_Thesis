...
sdm2_scaled_stats <- sdm2_stats %>%
  mutate(across(c(age, lag_PA, lag_G, lag_wOBA, AVG, OBP, SLG, ISO, OPS), scale))


metrics <- c("lag_PA", "lag_G", "lag_wOBA", "AVG", "OBP", "SLG", "ISO", "OPS")
...
msi_only_inury <- coxph(
  Surv(start, stop, event) ~ 
    ns(lag_PA, df = 3) +
    lag_total_injuries,
  data = sdm2_lag_injury
)
summary(msi_only_inury)
BIC(msi_no_missed, msi_with_injury)
anova(msi_no_missed, msi_with_injury, test = "LRT")
